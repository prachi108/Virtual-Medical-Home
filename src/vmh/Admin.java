	...
	public void insertRecord(){
		...
		try{
			 ...
			 int rowAffected2 = -1;
			    ...
			    pstmt1=conn.prepareStatement("insert into admin_profile(id,name,fname,nationality,dob,gender," +
...
public ArrayList getAdminList(){
		...
		try{
			 conn=DBConnection.getConnection();
			 String sql="SELECT id,name,gender,city1,bloodGroup FROM admin_profile order by id";
			...
			sql="SELECT status FROM login where userType = 'admin' order by id";
...
public void updateRecord(){
	...
	try{
		...
		pstmt=conn.prepareStatement("update admin_profile set name = ?,fname = ?,nationality = ?,dob = ?," +
...
public void getRecord(){
	...
	try{
		 ...
		 String sql="SELECT id,name,fname,nationality,dob,gender,address1,city1,state1,country1,zip1,landline1,mobile1," +
		 		...
		 		"email,alternateEmail,registrationDate,createdBy FROM admin_profile where id=?";
		pstmt1=conn.prepareStatement(sql);
		pstmt1.setString(1,this.id);
		rset1=pstmt1.executeQuery();
		if(rset1.next()){
