...
<tr height="475px">
    <td><div style="overflow:auto; height:450px; "><table width="1200" align="center">
      <tr>
        <td colspan="7"><center><h3><strong><span class="lblHeading">Doctor List</span></strong></h3></center></td>
      </tr>
	  <% if(session.getAttribute("message")!= null){ %> 
		<tr>
          <td colspan="7" style="color:#CC0000" align="center"><%=session.getAttribute("message")%></td>
        </tr>
		<%
	  ...
	  ArrayList list = doctor.getDoctorList();
      ...
      <tr  bgcolor=<%=i%2==0?"#FFFBF0":"#CCCCFF"%>>
        ...
        <td style="width:300px;"><a href="DoctorEdit.jsp?id=<%=userNameList.get(i)%>">Edit </a>| 
        <a href="DoctorView.jsp?id=<%=userNameList.get(i)%>">View</a>|
		<a href="DoctorDisable.jsp?id=<%=userNameList.get(i)%>&status=<%=statusList.get(i)%>"><%=statusList.get(i)%></a>|
		<a href="DoctorResetPassword.jsp?id=<%=userNameList.get(i)%>">Reset Password</a>|
		<a href="DoctorSpecializationEdit.jsp?id=<%=userNameList.get(i)%>">Specialization</a>		
		</td>
      </tr>
	  <%}
	  }else{
